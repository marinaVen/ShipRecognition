...
for act, name in [(torch.nn.Sigmoid(), "Sigmoid"),
                  (torch.nn.Tanh(), "Tanh"),
    ...
    val_loss, val_acc, train_loss, train_acc = train_model(net, loss, optimizer, scheduler, num_epochs=20)

    ax1.plot(val_loss, label="{} Validation Loss".format(name))
    ax0.plot(val_acc, label="{} Validation Accuracy".format(name))
    ax2.plot(train_loss, label="{} Train Loss".format(name))
    ax3.plot(train_acc, label="{} Train Accuracy".format(name))
