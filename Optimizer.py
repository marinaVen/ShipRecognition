...
for optimizer, name in [(torch.optim.SGD(AlexNet3(act).to(device).parameters(), lr=1.0e-2), "SGD"),
                        (torch.optim.Adam(AlexNet3(act).to(device).parameters(), lr=1.0e-3), "Adam"),
                        (torch.optim.RMSprop(AlexNet3(act).to(device).parameters(), lr=1.0e-3), "RMSprop"),
                        (torch.optim.SGD(AlexNet3(act).to(device).parameters(), lr=1.0e-3, momentum=0.9), "optim.SGD")]:
...
ax1.grid()
fig1.show()
fig1.savefig("Validation Loss")

ax0.legend()
ax0.set_title('Точность')
ax0.grid()
fig0.show()
fig0.savefig("Validation Accuracy")

ax2.legend()
ax2.set_title('Функция потерь')
ax0.grid()
fig2.show()
fig2.savefig("Train Loss")

ax3.legend()
ax3.set_title('Точность')
ax0.grid()
fig3.show()
fig3.savefig("Train Accuracy")
