...
class AlexNet(torch.nn.Module):
    def __init__(self, act):
        ...
        self.conv1 = torch.nn.Conv2d(1, 96, 11, stride=4, padding=2)
        self.act1 = act
        self.batch_norm1 = torch.nn.BatchNorm2d(96)
        self.pool1 = torch.nn.MaxPool2d(3, 2)

        self.conv2 = torch.nn.Conv2d(96, 256, 5, stride=1, padding=2)
        self.act2 = act
        self.batch_norm2 = torch.nn.BatchNorm2d(256)
        self.pool2 = torch.nn.MaxPool2d(3, 2)

        self.conv3 = torch.nn.Conv2d(256, 384, 3, stride=1, padding=1)
        self.act3 = act
        self.batch_norm3 = torch.nn.BatchNorm2d(384)

        self.conv4 = torch.nn.Conv2d(384, 384, 3, stride=1, padding=1)
        self.act4 = act
        self.batch_norm4 = torch.nn.BatchNorm2d(384)

        self.conv5 = torch.nn.Conv2d(384, 256, 3, stride=1, padding=1)
        self.act5 = act
        self.batch_norm5 = torch.nn.BatchNorm2d(256)
        self.pool5 = torch.nn.MaxPool2d(3, 2)
        ...
        self.act7 = act
        self.batch_norm7 = torch.nn.BatchNorm1d(2048)

        self.fc3 = torch.nn.Linear(2048, 12)
...
for act, name in [(torch.nn.Sigmoid(), "Sigmoid"), (torch.nn.Tanh(), "Tanh"), (torch.nn.LeakyReLU(), "LeakyReLU"),
                  (torch.nn.ReLU(), "ReLU")]:
    net = AlexNet(act)
    net = net.to(device)
    ...
    ax1.plot(val_loss, label=name)
    ax0.plot(val_acc, label=name)
    ax2.plot(train_loss, label=name)
    ax3.plot(train_acc, label=name)

ax1.legend()
ax1.set_title('Функция потерь')
ax1.grid()
fig1.show()
fig1.savefig("Validation Loss5 ")

ax0.legend()
ax0.set_title('Точность')
ax0.grid()
fig0.show()
fig0.savefig("Validation Accuracy5 ")

ax2.legend()
ax2.set_title('Train Loss')
fig2.show()
fig2.savefig("Train Loss5 ")

ax3.legend()
ax3.set_title('Train Accuracy')
fig3.show()
fig3.savefig("Train Accuracy5 ")
